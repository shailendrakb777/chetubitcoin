...
const char * const H2HCOIN_CONF_FILENAME = "h2hcoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "h2hcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\H2hcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\H2hcoin
    // Mac: ~/Library/Application Support/H2hcoin
    // Unix: ~/.h2hcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "H2hcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/H2hcoin";
#else
    // Unix
    return pathRet / ".h2hcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", H2HCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure H2hcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("H2hcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The H2hcoin Core developers";
    }
