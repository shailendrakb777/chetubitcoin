This directory contains the H2hcoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the H2hcoin UI.
...
ClientModel is used by the main application `h2hcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/h2hcoin/h2hcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a h2hcoin:... link from a browser).
...
* UI elements like H2hcoinAmountField, which inherit from QWidget.
* `h2hcoinstrings.cpp`: automatically generated
* `h2hcoinunits.(h/cpp)`: H2H / mBTC / etc handling
* `callback.h`
...
* don't change `local/h2hcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
4. Enter "h2hcoin-qt" as project name, enter src/qt as location
5. Leave the file selection as it is
6. Confirm the "summary page"
7. In the "Projects" tab select "Manage Kits..."
8. Select the default "Desktop" kit and select "Clang (x86 64bit in /usr/bin)" as compiler
9. Select LLDB as debugger (you might need to set the path to your installation)
10. Start debugging with Qt Creator (you might need to the executable to "h2hcoin-qt" under "Run", which is where you can also add command line arguments)
