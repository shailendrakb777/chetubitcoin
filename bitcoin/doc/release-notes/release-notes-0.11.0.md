H2hcoin Core version 0.11.0 is now available from:

  <https://h2hcoin.org/bin/h2hcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer H2H/kB fee to be
...
For example, add the following to `h2hcoin.conf`:
...
related to the blockchain in the h2hcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows H2hcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that H2hcoin Core will 
...
For an example script see [h2hcoin-submittx](https://github.com/laanwj/h2hcoin-submittx).
...
- #5334 `cf87536` libh2hcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new h2hcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only H2hcoinAmountField
- #5753 `0f44672` Add h2hcoin logo to about screen
...
- #5809 `46bfbe7` Add h2hcoin-cli man page
...
- #5849 `41259ca` contrib/init/h2hcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename H2hcoin-Qt.app to H2hcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for h2hcoin-tx)
