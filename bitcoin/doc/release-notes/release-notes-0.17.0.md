H2hcoin Core version 0.17.0 is now available from:

  <https://h2hcoincore.org/bin/h2hcoin-core-0.17.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://h2hcoincore.org/en/list/announcements/join/>
...
installer (on Windows) or just copy over `/Applications/H2hcoin-Qt` (on Mac)
or `h2hcoind`/`h2hcoin-qt` (on Linux).
...
H2hcoin Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer (Windows XP is not supported).

H2hcoin Core should also work on most other Unix-like systems but is not
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `h2hcoin.conf` file, not inside included files or from
  ...
  as h2hcoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the h2hcoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=h2hcoin
    test.uacomment=h2hcoin-testnet
...
be used in V0.17 by starting h2hcoind with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed H2hcoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/h2hcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for H2hcoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://h2hcointalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed H2hcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid H2hcoin transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in H2H. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When h2hcoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If h2hcoin is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart h2hcoind with
...
- When running h2hcoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If h2hcoind
...
- #11962 `0a01843` add seed.h2hcoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in H2H (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI h2hcoin:// (krab)
...
- #13528 `b877c39` Move H2hcoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libh2hcoin-util to libh2hcoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for h2hcoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling h2hcoin-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use H2hcoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to h2hcoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use H2hcoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use H2hcoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use H2hcoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for H2hcoinTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_h2hcoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.h2hcoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_h2hcoin` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in h2hcoind systemd service file (Flowdalic)
- #12652 `55f490a` h2hcoin-cli: Provide a better error message when h2hcoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to h2hcoind in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for h2hcoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` h2hcoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix h2hcoin-cli --version (Empact)
...
- #12877 `2b54155` Use h2hcoind in Tor documentation (knoxcard)
...
- #13346 `60f0358` update h2hcoin-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust h2hcoincore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed h2hcoin-cli -help output for help2man (hebasto)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/h2hcoin/).
