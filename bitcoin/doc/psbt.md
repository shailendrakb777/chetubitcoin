# PSBT Howto for H2hcoin Core

Since H2hcoin Core 0.17, an RPC interface exists for Partially Signed H2hcoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/h2hcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for H2hcoin transactions that are not fully signed
...
#### Multisig with multiple H2hcoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
H2hcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `h2hcoin-cli` is used.
...
Later, when *V* H2H has been received on *Amulti*, and Bob and Carol want to
