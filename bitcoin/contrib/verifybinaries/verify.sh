...
###   h2hcoincore.org and h2hcoin.org and compares them.
...
WORKINGDIR="/tmp/h2hcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://h2hcoincore.org"
HOST2="https://h2hcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="h2hcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'h2hcoin-',
...
if [ $? -ne 0 ]; then
   echo "h2hcoin.org failed to provide signature file, but h2hcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "h2hcoin.org and h2hcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the H2hcoin Core binary release signing key installed?"
   fi
